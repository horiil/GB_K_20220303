...
public class BasicStudentController {
	...
	public String save(@ModelAttribute("student")Student student, RedirectAttributes redirectAttributes) {
		...
		redirectAttributes.addAttribute("SId", savedSList.getStudentId());
	...
	public String student(@PathVariable Long SId , Model model) {
	...
	@GetMapping("/{SId}")
	public String item(@PathVariable long SId  ,Model model) {
		Student student = studentRepository.findById(SId);
		model.addAttribute("student", student);
		return "basic/student";
	}
